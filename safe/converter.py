...
class SAFEConverter:
    ...
    def decoder(
        ...
        # XXX added this line because 'mol' can be the input invalid SMILES
        if isinstance(mol, str):
            return mol

        if remove_dummies:
            # with suppress(Exception): # XXX changed this line
...
def decode(
    ...
    """
    # with dm.without_rdkit_log(): # XXX Edited to handle errors
    ...
    #     if ignore_errors:
    #         return None
    #     # raise SAFEDecodeError(f"Failed to decode {safe_str}") from e # FIXME changed this line to return error
    #     return str(e)
